...
def restore_inventory_on_order_item_delete(sender, instance, **kwargs):
    """
    Kapag na-delete ang isang OrderItem (hal. tinanggal ng admin sa order,
    hindi dahil na-cancel ang buong order) — i-restore ang ingredients nito
    para hindi permanenteng nabawasan ang stock para sa item na hindi na
    bibilangin sa order.
    """
    product  = instance.product
    ...
    for ingredient in ingredients:
        ...
        StockMovement.objects.create(
            ...
            unit_cost=inventory_item.cost_per_unit,
            reference=f'Order #CC-{str(instance.order_id).zfill(5)}',
            notes=f'Restored — {product.name} x{quantity} removed from order.',
        )
...
def restore_inventory_on_cancel(sender, instance, created, **kwargs):
    """
    Kapag nagbago ang status papuntang 'cancelled' — i-restore ang inventory
    ng ingredients ng lahat ng items sa order.
    """
    if created:
        return

    # Only trigger kapag bagong na-cancel (hindi na-restore ulit kung
    # cancelled na siya dati, at hindi rin kung hindi cancelled ang bagong status)
    old_status = getattr(instance, '_old_status', None)
