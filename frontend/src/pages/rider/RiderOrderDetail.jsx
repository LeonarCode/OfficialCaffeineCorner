...
const RiderOrderDetail = () => {
  ...
  const hasCoords = order.delivery_latitude && order.delivery_longitude
  const lat = hasCoords ? parseFloat(order.delivery_latitude) : null
  const lng = hasCoords ? parseFloat(order.delivery_longitude) : null
  ...
  return (
    <div className='min-h-screen bg-[#FAF6F0] pb-6'>
      ...
      <div className='bg-[#2C1503] px-5 py-5 flex items-center gap-3 sticky top-0 z-10'>
        <button onClick={() => navigate('/rider/dashboard')} className='text-white text-lg'>Back</button>
        <div>
          <p className='text-white font-bold text-sm'>Order #{order.id}</p>
          <p className='text-[#C4A882]/50 text-xs'>{ORDER_TYPE_LABELS[order.order_type] || 'Regular'} Order</p>
        </div>
      </div>
      ...
      <div className='px-4 py-4 flex flex-col gap-4'>
        ...
        <div className='bg-white rounded-2xl p-4 shadow-sm'>
          ...
          <div className='flex flex-col gap-2 text-sm'>
            ...
            <p className='text-[#2C1503] font-bold text-base'>{order.customer_name}</p>
            ...
            {order.notes && (
              <div className='flex items-start gap-2 bg-amber-50 rounded-xl p-2 mt-1'>
                <span className='text-amber-700 text-xs'>Notes: {order.notes}</span>
              </div>
            )}
          </div>
        </div>
        ...
        {hasCoords && (
          <div className='bg-white rounded-2xl overflow-hidden shadow-sm'>
            <div className='p-4 pb-2'>
              <p className='text-[#2C1503] font-semibold text-sm'>Delivery Location</p>
            </div>
            ...
            <a
              ...
              className='flex items-center justify-center gap-2 bg-[#2C1503] text-white text-sm font-bold py-3 hover:bg-[#5a2f00] transition'
            >
              Open in Google Maps
            </a>
          </div>
        ...
        <div className='bg-white rounded-2xl p-4 shadow-sm'>
          ...
          {order.items.map(item => (
            <div key={item.id} className='flex justify-between text-sm py-1.5 border-b border-gray-50 last:border-0'>
              ...
              <span className='text-[#2C1503] font-semibold'>P{parseFloat(item.subtotal).toFixed(2)}</span>
            </div>
          ))}
          <div className='flex justify-between text-sm pt-2 mt-2 border-t border-gray-100'>
            ...
            <span className='font-bold text-[#2C1503]'>P{parseFloat(order.total_price).toFixed(2)}</span>
          </div>
          <div className='flex items-center gap-2 mt-2'>
            <span className='text-xs bg-gray-100 px-2 py-0.5 rounded-full'>
              {order.payment_method === 'cod' ? 'Cash' : 'GCash'}
            </span>
